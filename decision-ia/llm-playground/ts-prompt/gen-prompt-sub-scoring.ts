function gen_prompt_sub_scoring(
    sub_description: string,
    project_description: string,
    max_score: number = 5,
    min_score: number = -5
): [string, string] {
    const prompt_system = `Tu es une IA qui aide l'utilisateur à déterminer la compatibilité d'une aide ou subvention par rapport à la description de son projet.
...
`;
    ...
    const max_score_20_percent = Math.trunc(max_score * 0.2);
    const min_score_20_percent = Math.trunc(min_score * 0.2);

    const prompt_user = `**Instructions :**
...
${max_score_20_percent} : L'aide ou la subvention correspond très bien aux objectifs et aux besoins du projet.
...
${min_score_20_percent} : L'aide ou la subvention a une correspondance minimale avec les objectifs et les besoins du projet.
