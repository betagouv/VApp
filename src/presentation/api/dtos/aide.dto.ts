...
export const aideDtoSchema = z.object({
  ...
  fournisseurDonnees: z.nativeEnum(FournisseurDonneesAides).optional(),
  ...
  urlDescriptif: z.string().url().optional().describe('Lien vers la page descriptive de l’aide'),
...
aideDtoSchema._output satisfies Omit<AideInterface, 'types'>;

export type AideDto = z.infer<typeof aideDtoSchema>;

export const aideEvalueeDtoSchema = z
  .object({
    scoreCompatibilite: z.number().int().min(0).max(100).describe('Score de compatibilité interne, sur 100')
  })
  .merge(aideDtoSchema);

aideEvalueeDtoSchema._output satisfies AideEvalueeInterface;

export type AideEvalueeDto = z.infer<typeof aideEvalueeDtoSchema>;
