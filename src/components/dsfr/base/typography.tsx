...
const typographyProps = <P extends Omit<TypographyProps, "children">>({
  ...
  className: cx(
    fr.cx(
      buildSpacingClasses({ mt, mr, mb, ml, mx, my, pt, pr, pb, pl, px, py }),
    ),
    className,
  ),
  ...rest,
...
type HeadingAttributes = PropsWithChildren<
...
export const H1 = ({ as: As, ...rest }: HeadingProps) => (
  <Heading tag="h1" as={As} {...boxProps(rest)} />
);
export const H2 = ({ as: As, ...rest }: HeadingProps) => (
  <Heading tag="h2" as={As} {...boxProps(rest)} />
);
export const H3 = ({ as: As, ...rest }: HeadingProps) => (
  <Heading tag="h3" as={As} {...boxProps(rest)} />
);
export const H4 = ({ as: As, ...rest }: HeadingProps) => (
  <Heading tag="h4" as={As} {...boxProps(rest)} />
);
export const H5 = ({ as: As, ...rest }: HeadingProps) => (
  <Heading tag="h5" as={As} {...boxProps(rest)} />
);
export const H6 = ({ as: As, ...rest }: HeadingProps) => (
  <Heading tag="h6" as={As} {...boxProps(rest)} />
);

type TextVariant = TypoVariant extends infer R
  ? R extends `.fr-text--${infer T}`
    ? T
    : never
  : never;
...
const textProps = ({
  variant,
  ...rest
}: Omit<TextProps<boolean>, "children" | "inline">) => {
  const tagProps = typographyProps(rest);
  tagProps.className = cx(
    tagProps.className,
    cx(variant && [variant].flat().map((v) => `fr-text--${v}`)),
  );
 ...
 */
export const Text = <Inline extends boolean>({
  inline,
  children,
  ...rest
}: TextProps<Inline>) =>
  inline ? (
    <span {...textProps(rest)}>{children}</span>
  ) : (
    <div {...textProps(rest)}>{children}</div>
  );

/**
 * Ref version of {@link Text}
 */
export const TextRef = forwardRef<
