...
export default async function Page({
  ...
  const aides = await Promise.all(
    projet.recommendations.map(({ aideId }) =>
      dummyAideRepository.fromUuid(aideId),
    ),
  );
  const recommendations = projet.recommendations.map(
    ({ eligibilite }, index) => ({ eligibilite, aide: aides[index] }),
  );
