...
Votre tâche est d'analyser la correspondance entre l'aide et le projet, et de fournir un **score sous forme d'entier entre ${NOTE_MIN} et ${NOTE_MAX}**.
**Répondez uniquement par le nombre entier représentant le score, sans aucun autre texte ou caractère supplémentaire.**`;

export const user = (aide: Aide, projet: Projet) => `[Système de scoring]
Attribuez une note entière entre  ${NOTE_MIN} et ${NOTE_MAX}, selon les critères suivants :
...
- ${0.9 * (NOTE_MAX - NOTE_MIN) + NOTE_MIN}  : L'aide correspond très bien aux objectifs et aux besoins du projet.
...
- ${0.8 * (NOTE_MAX - NOTE_MIN) + NOTE_MIN}  : L'aide correspond bien aux objectifs et aux besoins du projet.
...
- ${0.2 * (NOTE_MAX - NOTE_MIN) + NOTE_MIN}  : L'aide correspond très peu aux objectifs et aux besoins du projet.
