 ...
 */
export type ListEventEmitterEvents<TEE extends EventEmitter> =
  TEE["on"] extends (event: infer T extends string, listener: Any) => void
    ? T extends string
      ? T
      : never
    : never;
...
export type GetEventListener<
  TEE extends EventEmitter,
  TEvent extends ListEventEmitterEvents<TEE>,
> = TEE["on"] extends (event: infer T, listener: infer TListener) => void
  ? T extends TEvent
    ? TListener
    : never
  : never;
...
>;
export type PopUnion<TUnion> =
  UnionToOverloads<TUnion> extends (a: infer A) => void ? A : never;
...
> = true extends N ? [] : PushToArray<TuplifyUnion<Exclude<T, L>>, L>;
...
export type PropsWithoutChildren<T extends PropsWithChildren> = Omit<
  T,
  "children"
>;
 ...
 */
export const ensureRequired = <T>(value: T) =>
  value as Required<NonNullableProps<T>>;
